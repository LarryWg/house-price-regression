...
from src.data_preprocess.load_data import load_data
from src.data_preprocess.preprocess import (
    clean_merge, duplicated_data, outliers, preprocess
)
from src.features.feature_engineering import (
    date_features, frequency_encoding, target_encoding, drop_unused_columns, run_feature_engineering
)
...
def test_load_and_split_data_creates_splits(tmp_path):
    ...
    train, eval, holdout = load_data(file_name=str(dummy_path), output_file=tmp_path)
...
def test_remove_outliers_drops_high_prices():
    df = pd.DataFrame({"median_listing_price": [100_000, 500_000, 20_000_000]})
    cleaned = outliers(df)
    assert cleaned["median_listing_price"].max() <= 19_000_000
...
def test_clean_and_merge_skips_when_city_missing():
    ...
    result = clean_merge(df, metros_path=None)  # should skip gracefully
...
def test_add_date_features_extracts_parts():
    ...
    df = date_features(df)
    assert df.loc[0, "year"] == 2020 and df.loc[0, "month"] == 1 and df.loc[0, "day"] == 15
...
def test_frequency_encode_counts_values():
    ...
    eval_df = pd.DataFrame({"zipcode": [1000, 3000]})
    train, eval_df, freq_map = frequency_encoding(train, eval_df, "zipcode")
    assert train["zipcode_freq"].tolist() == [2, 2, 1]
    assert eval_df["zipcode_freq"].iloc[0] == 2
    assert pd.isna(eval_df["zipcode_freq"].iloc[1])  # unseen 3000 -> NaN from .map()
...
def test_target_encode_applies_mapping():
    ...
    train, eval, te = target_encoding(train, eval, "city_full", "price")
...
def test_full_pipeline_integration(tmp_path):
    ...
    train, eval, holdout = load_data(file_name=str(raw_path), output_file=tmp_path)
    ...
    preprocess("train", raw_dir=tmp_path, processed_dir=processed_dir, metros_path=None)
    preprocess("eval", raw_dir=tmp_path, processed_dir=processed_dir, metros_path=None)
    preprocess("holdout", raw_dir=tmp_path, processed_dir=processed_dir, metros_path=None)
